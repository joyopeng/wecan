...
//      canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
    ...
    private void drawCellText(Canvas canvas, int index, String text, int color) {
        ...
        String sss = "";//mScheduleEmployeeNames.get(Integer.valueOf(text) - 1);
        if(mScheduleEmployeeNames == null || mScheduleEmployeeNames.size() <= 0){
            sss = "未排班";
        }else{
            sss = mScheduleEmployeeNames.get(Integer.valueOf(text) - 1);
            if(sss.isEmpty()){
                sss = "未排班";
            }
        }
        float textY = surface.monthHeight + surface.weekHeight + (y - 1)
                * surface.cellHeight + surface.cellHeight * 3 / 4f;
        float textX = (surface.cellWidth * (x - 1))
                + (surface.cellWidth - surface.datePaint.measureText(sss))
                / 2f;
        if(surface.datePaint.measureText(sss) > surface.cellWidth){
            textX = (surface.cellWidth * (x - 1))
                    + (surface.cellWidth - surface.datePaint.measureText(sss.substring(0,6)+"..."))
                    / 2f;
            canvas.drawText(sss.substring(0,6)+"...", textX, textY,"未排班".equals(sss) ? surface.dateGrayPaint : surface.datePaint);
        }else{
            canvas.drawText(sss, textX, textY,"未排班".equals(sss) ? surface.dateGrayPaint : surface.datePaint);
        }
    ...
    public String getYearAndmonth() {
        ...
        Log.i(TAG,"getYearAndmonth year = " + year + ";month = " + month);
    ...
    private ArrayList<String> mScheduleEmployeeNames = null;
    public void setDaysShow(ArrayList<String> names){
        mScheduleEmployeeNames = names;
    }

    public String clickToMonth(int year,int month){
        Log.i("zhangsh","clickToMonth year = " + year + ";month = " + month);
        calendar.setTime(curDate);
        ...
        curDate = calendar.getTime();
        Log.i(TAG,"clickToMonth dayInWeek = ");
        invalidate();
        return getYearAndmonth();
    }
...
//              }
                    selectedStartDate = selectedEndDate = downDate;
                    //响应监听事件
                    Log.i(TAG,"zhangsh onTouch selectedStartDate = " + selectedStartDate);
    ...
    private class Surface {
        ...
        public void init() {
            ...
            datePaint.setTextSize(cellTextSize*0.5f);
            //datePaint.setTypeface(Typeface.DEFAULT_BOLD);
            datePaint.setTypeface(Typeface.DEFAULT);
            dateGrayPaint = new Paint();
            dateGrayPaint.setColor(Color.GRAY);
            dateGrayPaint.setAntiAlias(true);
            dateGrayPaint.setTextSize(cellTextSize*0.5f);
            dateGrayPaint.setTypeface(Typeface.DEFAULT);
            boxPath = new Path();
            ...
            //boxPath.rLineTo(0, height - monthHeight);
            boxPath.moveTo(7 * cellWidth, monthHeight + weekHeight);
            boxPath.rLineTo(0, height - monthHeight);
            boxPath.moveTo(0, monthHeight + weekHeight);
            boxPath.rLineTo(0, height - monthHeight);
            boxPath.moveTo(0, monthHeight + weekHeight + 7*cellHeight);
            boxPath.rLineTo(width, 0);
     ...
     * set default date
     * */
    public void setDefaultItemClick(Date date){
