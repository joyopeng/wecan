...
public class AppExceptionHandler implements UncaughtExceptionHandler {
	...
	public static final String TAG = "AppExceptionHandlder";
	...
	public void init(Context context) {
		...
		// 设置该AppExceptionHandlder为程序的默认处理器
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		if (!handleException(ex) && mDefaultHandler != null) {
			...
			System.exit(1);
//			
//			Intent intent = new Intent(mContext, MobileLoginActivity.class);
...
//			mContext.startActivity(intent); 
	...
	private void saveCrashInfo2File(Throwable ex) {
		...
		try {
			...
			String fileName = "error_" + DateTimeUtil.DateFormat(new Date(), "yyyy_MM_dd") + ".log";
			...
			String dtStr = DateTimeUtil.DateFormat(new Date(),"yyyy-MM-dd HH:mm:ss");
			bw.write(dtStr +"\n"+sb.toString() );
			bw.close();
			writer2.close();
//			String time = formatter.format(new Date());
//			String fileName = "error_" + time + "-" + timestamp + ".log";
//			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
//				String path = FileUtil.getPath() + "/logs";
//				File dir = new File(path);
//				if (!dir.exists()) {
//					dir.mkdirs();
//				}
